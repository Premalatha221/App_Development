...
import baby from '../src/assets/images/baby.jpg'; // Update the path as needed
...
const AdminProfile = () => {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      console.log('Updated Profile:', updatedProfile); // Log the data being sent
      console.log('API Endpoint:', `http://127.0.0.1:8080/api/users/updateUser/${email}`); // Log the endpoint
      ...
      if (response.status === 200) {
        ...
        console.error('Error updating profile data:', response.status, response.data);
      }
    } catch (error) {
      console.error("Error updating profile data:", error.response || error.message);
    }
  };
