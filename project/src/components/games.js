...
const GamesPage = () => { 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredData = response.data.filter(product => product.category === "games");
  ...
  const handleAddToWishlist = (product) => {
    if (userRole === 'ADMIN') {
      showSnackbar('Admins do not have permission to add to Wishlist');
      return;
    }
  ...
  const handleAddToCart = (product) => {
    if (userRole === 'ADMIN') {
      showSnackbar('Admins do not have permission to add to Cart');
      return;
    }
    setCart((prevCart) => {
  ...
  const handleBuy = (product) => {
    if (userRole === 'ADMIN') {
      showSnackbar('Admins do not have permission to Buy');
      return;
    }
    if (isLoggedIn) {
